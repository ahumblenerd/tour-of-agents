...
export const semanticKernel: FrameworkComparison = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: "The Kernel is a config object that holds your API key and a dict of tools. A KernelFunction is a regular function in that dict. The Planner is a system prompt instruction — tell the LLM to break the task into steps and it will, no planner class needed. Memory is a list of strings you embed and search, or just a dict you inject into the prompt. Orchestration is the same while loop every agent uses: call the LLM, check if the response has tool_calls, look up the function in your tools dict, call it, append the result, repeat. The filter pipeline is a try/except around your function calls. The entire agent — including plugin dispatch, planning, and memory — is about 60 lines. No Kernel object, no plugin registry, no planner hierarchy.",
    },
    {
