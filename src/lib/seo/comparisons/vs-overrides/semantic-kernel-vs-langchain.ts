...
Use Semantic Kernel when the agent has to live inside a Microsoft estate — C# services calling \`Kernel.invoke()\`, Azure AI Search for memory, filters for audit logging. Use LangChain when you're stitching together many integrations in Python: a vector store for RAG, multiple model providers behind one interface, \`LangGraph\` for branching workflows, \`LangSmith\` for traces. SK is heavier on orchestration ceremony per call; LangChain is heavier on dependency surface across the project. Microsoft is also folding SK toward the Microsoft Agent Framework, so roadmap risk differs: LangChain owns its own direction, SK is being repositioned alongside a sibling product.`,
