...
const copy: VsCopy = {
  headToHead: `CrewAI models work as a team metaphor: \`Agent(role, goal, backstory)\` plus \`Task\` plus \`Crew(process=sequential|hierarchical)\`. Pydantic AI models work as a typed function call: \`Agent\` with a \`result_type\` Pydantic model, \`@agent.tool\` decorators with typed parameters, and \`RunContext[DepsType]\` for dependency injection.
