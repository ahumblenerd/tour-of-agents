...
const copy: VsCopy = {
  headToHead: `AutoGPT is a **finished autonomous agent** — you give it a goal, its \`Agent\` class decomposes it, runs a think-plan-act-observe loop, and decides on its own when to stop. LangChain is a **toolkit of primitives** — \`AgentExecutor\`, \`@tool\`, \`ConversationBufferMemory\`, \`OutputParser\` — that you compose into whatever loop you want.
...
Use AutoGPT when the task is open-ended and you want the agent to figure out its own subtasks — research, exploration, multi-step automation where you cannot enumerate steps upfront. Expect dozens of LLM calls per run.

Use LangChain when the task is bounded but the **integration count is high** — RAG over a specific vector store, swapping providers, production tracing via \`LangSmith\`, or branching state machines via \`LangGraph\`. You write the loop; the framework supplies the connectors.`,
