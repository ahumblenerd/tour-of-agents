...
export const mastra: FrameworkComparison = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: "An agent is an async function that POSTs to the LLM API, checks for tool_calls in the response, executes matching functions from a tools object, and loops. Workflows are async functions that call other async functions with if/else branching — no framework needed to run step A, then step B, then branch on a condition. RAG is three operations: call an embedding API, store vectors in an array (or database), and find the closest match with cosine similarity. Memory is a messages array you persist to a file or database. Studio is console.log and a test file. The entire agent — tools, memory, RAG retrieval — fits in about 60 lines of TypeScript. No classes, no decorators, no build step. Just functions, objects, and fetch calls.",
    },
    {
