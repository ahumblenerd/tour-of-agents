...
export const camelAi: FrameworkComparison = {
  ...
  rows: [
    ...
    { concept: "Society", framework: "Multi-agent societies with role assignment, communication, and voting", plain: "A loop over N agents, each with a different system prompt, sharing a message list" },
  ...
  sections: [
    ...
    },
    {
      ...
      body: "Role-playing in plain Python is two LLM calls per turn with different system prompts. The instructor call gets a prompt like 'You are a project manager. Break this task into steps and give the next instruction.' The assistant call gets 'You are a developer. Execute the instruction and report the result.' Both share a messages list so each sees what the other said. Inception prompting is just a detailed system prompt that prevents role drift — include the task, the role, and behavioral constraints. A society of agents is a for loop over N agents with different prompts, each appending to a shared conversation. The entire multi-agent debate pattern fits in about 50 lines. The insight is in the prompting technique, not the code.",
    },
    {
      heading: "When to use CAMEL AI",
      body: "CAMEL AI is valuable for research on multi-agent systems — studying how agent societies form consensus, how role assignment affects output quality, and how communication patterns scale. If you are running experiments on agent collaboration, the framework provides built-in infrastructure for role-playing sessions, conversation logging, and society simulation. It also helps when you need structured multi-agent debate for production tasks: having an analyst and a critic review each other's work genuinely improves output quality on complex reasoning tasks. The academic origin means the framework is well-documented in papers with reproducible experimental setups.",
    },
    {
