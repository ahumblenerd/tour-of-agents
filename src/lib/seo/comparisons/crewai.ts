...
export const crewai: FrameworkComparison = {
  ...
  sections: [
    {
      ...
      body: "CrewAI models multi-agent systems as a **crew of specialists**. Each `Agent` has a role (`\"Senior Researcher\"`), a goal (`\"Find the best data sources\"`), a backstory that shapes its behavior, and a set of tools it can use. `Tasks` define discrete units of work with expected outputs. The `Crew` orchestrates execution — sequentially, hierarchically, or with a custom process.\n\nCrewAI also provides memory systems:\n- `ShortTermMemory`\n- `LongTermMemory`\n- `EntityMemory`\n\nIt also supports delegation, where one agent can hand off subtasks to another. The mental model is **a team of people collaborating on a project**. For prototyping multi-agent workflows where you want to reason about roles and responsibilities, it provides a clean vocabulary.",
    },
    {
      heading: "The plain Python equivalent",
      body: "An `Agent` in CrewAI is a function with a system prompt that includes the role, goal, and backstory. The `tools` dict maps names to callables. Task delegation is a list of tasks processed in order — each task calls the assigned agent function with the task description appended to the `messages`. Hierarchical execution is a manager agent that decides which sub-agent to call next (just another tool choice). Memory is a dict injected into the system prompt.\n\nThe entire crew pattern — multiple agents, task queue, delegation — is a `for` loop over tasks, where each iteration calls the right agent function. **No `Crew` class, no `process` kwarg.** Just functions calling functions with a shared `state` dict passed between them.",
    },
    {
      heading: "When to use CrewAI",
      body: "CrewAI adds value when you **genuinely have multiple agents with distinct roles** and you want a clean way to define and orchestrate them. If you're building a content pipeline (researcher -> writer -> editor), a data analysis crew (collector -> analyst -> reporter), or any workflow where role separation matters for prompt quality, CrewAI's vocabulary helps.\n\nIt also provides **guardrails around delegation** — agents can only delegate to agents in their crew, preventing runaway loops. For teams building multi-agent prototypes who want to iterate on roles and task definitions without writing orchestration code, CrewAI gets you to a working demo fast.",
    },
    {
      heading: "When plain Python is enough",
      body: "Most \"multi-agent\" systems are **actually one agent calling different system prompts** depending on the task. If your agents share the same tools and differ only in their instructions, you don't need a framework — you need different system prompt strings. If your task execution is always sequential, a `for` loop is clearer than `Crew(process=sequential)`.\n\nThe overhead of defining `Agent`, `Task`, and `Crew` objects adds ceremony without adding capability when the workflow is simple. Start with plain functions, add role separation through system prompts, and only reach for CrewAI when the **orchestration logic itself becomes the hard part** — not the prompts, not the tools, but the routing between agents.",
    },
  ],
  faqs: [
  ...
  references: {
    officialSite: "https://www.crewai.com/",
    docs: "https://docs.crewai.com/",
    github: "https://github.com/crewAIInc/crewAI",
    introBlog: "https://blog.crewai.com/",
    mcpRelevant: true,
    notable: [
      {
        title: "MCP Servers as Tools in CrewAI",
        url: "https://docs.crewai.com/en/mcp/overview",
        description: "Official documentation of CrewAI's first-class MCP integration.",
      },
      {
        title: "Crew AI with João Moura — Software Engineering Daily",
        url: "https://softwareengineeringdaily.com/2025/06/03/crew-ai-with-joao-moura/",
        description: "Long-form podcast interview with the founder on CrewAI's design and adoption.",
      },
      {
        title: "CrewAI Launches Multi-Agentic Platform — GlobeNewswire",
        ...
        description: "Press release marking the $18M funding and enterprise launch.",
      },
    ],
  },
};
